...
df_new = pd.concat([df[['ilink','ref_date_month','ref_date_year']],df_dummies],axis=1)
df_new = df_new.groupby(['ilink','ref_date_month','ref_date_year']).sum().reset_index()
...
select
  t3.ilink, date_part(mon,t3.ref_date) as ref_date_month, date_part(yr,t3.ref_date) as ref_date_year,
  max(t3.most_recent_past_order_date) as most_recent_past_order_date,
  max(t3.num_past_orders) as num_past_orders,
  max(t3.sum_past_shipped_sold_amt) as sum_past_shipped_sold_amt,
  max(t3.avg_past_shipped_sold_amt) as avg_past_shipped_sold_amt,
...
group by t3.ilink,ref_date_month,ref_date_year
order by t3.ilink, ref_date_month,ref_date_year;
"""
df = dbu.get_df_from_query(query)

print 'Joining Tables and Saving...'
df_new = df.merge(df_new,on=['ilink','ref_date_month','ref_date_year'])
...
df_new.to_pickle('../data/numericalFts_100kUsers_BaselineV1.pkl')
